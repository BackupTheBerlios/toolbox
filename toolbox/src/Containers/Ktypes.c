...
//======================================================
// $Id: Ktypes.c,v 1.2 2005/05/12 21:51:08 plg Exp $
//======================================================
...
inline tb_Key_t __cp_str(tb_Key_t K) { return (tb_Key_t)strdup(K.key); }
...
inline void __free_str(tb_Key_t K) { free(K.key); }
...
void register_basic_ktypes_once() {
	...
	__GLOBAL_KT = calloc(1, sizeof(struct __global_kt));
...
int registerNew_Ktype( char *name, 
	...
	ktype_t kt = calloc(1, sizeof(struct ktype));
	kt->name    = strdup(name);
	kt->keytype = n;
	kt->kcp     = kcp;
	kt->kcmp    = kcmp;
	kt->kfree   = kfree;
	kt->k2sz    = k2str;
	__GLOBAL_KT->ktypes = realloc(__GLOBAL_KT->ktypes, __GLOBAL_KT->nb*sizeof(ktype_t));
